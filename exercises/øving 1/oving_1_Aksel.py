...
class BayesianNetwork:
    ...
    def sorted_nodes(self):
        ...
        for variable in self.variables:
            if len(self.variables[variable].parents) == 0:
                S.append(self.variables[variable])
        num_parents = {}
        for var in self.variables:
            num_parents[var] = len(self.variables[var].parents)
        children = self.edges.copy()
        while len(S) != 0:
            n = S.pop(0)
            L.append(n.name)
            ...
            for m in self.edges[n]:
                ...
                for c in children[n]:
                    if c is not m:
                        child.append(c)
                num_parents[m.name] -= 1
                if num_parents[m.name] == 0:
                    S.append(m)
        # legger inn sjekk for syklisk graf her:
        for var in self.edges:
            if num_parents[var.name] >= 1:
                print("syklisk graf")
                return None
        return L
...
class InferenceByEnumeration:
    ...
    def _enumeration_ask(self, X, evidence):
        ...
        q = {}
        # ser på piazza at q skal være en liste, men lagde koden med q som dictionary og gjør den senere om til liste
        e_copy = evidence.copy()
        for xi in range(self.bayesian_network.variables[X].no_states):
            e_copy[X] = xi
            q[xi] = self._enumerate_all(
                self.topo_order, e_copy)

        return np.array(list(self.normalize(q).values()))
    ...
    def _enumerate_all(self, vars, evidence):
        ...
        if len(vars) == 0:
            return 1.0
        y = vars[0]
        node_y = self.bayesian_network.variables[y]
        if y in evidence:
            return (node_y.probability(evidence[y], evidence) * self._enumerate_all(vars[1:], evidence))
        else:
            summ = 0
            for i in range(node_y.no_states):
                e_copy = evidence.copy()
                e_copy[y] = i
                summ += node_y.probability(i, evidence) * \
                    self._enumerate_all(vars[1:], e_copy)
            return summ

    # hjelpefunksjon for å normalisere (summen av sannsynlighetene er lik 1)
...
def monty_hall():
    ...
    guest = Variable('ChosenByGuest', 3, [[1/3], [1/3], [1/3]])
    prize = Variable('Prize', 3, [[1/3], [1/3], [1/3]])
    host = Variable('OpenedByHost', 3, [[0, 0, 0, 0, 0.5, 1, 0, 1, 0.5], [0.5, 0, 1, 0, 0, 0, 1, 0, 0.5], [0.5, 1, 0, 1, 0.5, 0, 0, 0, 0]],
                    parents=['ChosenByGuest', 'Prize'],
                    no_parent_states=[3, 3])

    b = BayesianNetwork()

    b.add_variable(guest)
    b.add_variable(prize)
    b.add_variable(host)

    b.add_edge(guest, host)
    b.add_edge(prize, host)

    i = InferenceByEnumeration(b)
    post = i.query(
        'Prize', {'ChosenByGuest': 2, 'OpenedByHost': 0})

    print(f"Probability distribution, P(Prize | ChosenByGuest = 1, OpenedByHost = 3)")
    print(post)
